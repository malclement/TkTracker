...
under `~/.claude` (or `CLAUDE_CONFIG_DIR`), and writes its own scan cache to
...
untrusted transcript content — anything that makes the parser crash, hang, or
misattribute usage when fed a crafted JSONL line is in scope.
